...
# Config própria (sshd_config) para não herdar Port 22 da distro.
...
# apt install openssh-server costuma deixar :22 em 0.0.0.0. Uma vez, no start.
stop_sshd_port_22() {
  ...
  local line local_part port pid
  while IFS= read -r line; do
    ...
    [[ -n "${pid:-}" ]] || continue
    log "stopping distro sshd pid=$pid on :22"
...
is_listening() {
  local ip=$1
  ss -lntp 2>/dev/null | grep -qF "${ip}:${PORT}"
}
...
start_sshd() {
  ...
  if ! sudo "$BIN" -t -f "$CFG" -o "ListenAddress=$ip" >/dev/null 2>&1; then
    log "WARN: sshd -t failed; tentando iniciar mesmo assim"
  fi
  sudo setsid "$BIN" -D -e -f "$CFG" -o "ListenAddress=$ip" >>"$LOG" 2>&1 &
  local pid=$!
  sleep 1
